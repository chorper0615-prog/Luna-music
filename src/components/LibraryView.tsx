...
export default function LibraryView({ currentTrack, isPlaying, onPlay, favorites, isFavorite, onToggleFavorite }: Props) {
  const [history, setHistory] = useState<Track[]>(loadHistory);
  const [activeSection, setActiveSection] = useState<'recent' | 'favorites' | 'playlists'>('recent');
   ...
   <div className="w-full h-full overflow-y-auto no-scrollbar px-4 pb-4 pt-[env(safe-area-inset-top)]" style={{ touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
      ...
      <div className="mt-5 flex gap-2 rounded-[22px] border border-white/10 bg-white/8 p-1.5 backdrop-blur-xl">
        ...
        <button
          ...
          className={`flex-1 rounded-[18px] px-4 py-2.5 text-sm font-semibold transition-all duration-300 ${
            activeSection === 'favorites' ? 'bg-white text-black shadow-lg' : 'text-white/60'
          }`}
        >
          喜欢
        </button>
        ...
        favorites.length > 0 ? (
          <div className="mt-5 space-y-2">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => onPlay(favorites[0], favorites, 0)}
                className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-xs text-white/80 backdrop-blur-xl"
              >
            ...
            {favorites.map((track, index) => {
              ...
              return (
                <div
                  key={track.id}
                  className={`flex w-full items-center gap-3 rounded-[22px] border p-3 transition-all duration-300 ${
                    active ? 'border-white/20 bg-white/10' : 'border-white/8 bg-white/6 hover:bg-white/10'
                  }`}
                >
                  <button 
                  ...
                  >
                    <div className="relative h-14 w-14 overflow-hidden rounded-2xl shrink-0">
                      <AlbumArt track={track} isPlaying={active} size="md" className="h-14 w-14 rounded-2xl" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-white">{track.title}</p>
                  ...
                  >
                    <svg 
                      className="w-4 h-4" 
                      fill={isFavorite(track.id) ? '#ff6b81' : 'none'} 
                      stroke={isFavorite(track.id) ? '#ff6b81' : 'rgba(255,255,255,0.5)'} 
                      viewBox="0 0 24 24"
                      strokeWidth="2"
                    >
                      <path 
          ...
          <div className="mt-12 flex flex-col items-center text-center">
            <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-[26px] border border-white/10 bg-white/8 backdrop-blur-xl">
              <svg className="h-9 w-9 text-white/40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
            </div>
            <p className="text-lg font-semibold text-white">还没有喜欢的音乐</p>
            <p className="mt-1 text-sm text-white/55">去播放器里点爱心收藏喜欢的歌吧</p>
          </div>
        )
      ) : (
        <div className="mt-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: '我喜欢的', icon: '♥', color: 'from-rose-500 to-pink-600', count: `${favorites.length} 首`, action: () => setActiveSection('favorites') },
              { label: '最近添加', icon: '♪', color: 'from-slate-500 to-slate-700', count: `${history.length} 首` },
              ...
              <button
                key={i}
                onClick={playlist.action}
                className="group overflow-hidden rounded-[24px] border border-white/10 bg-white/8 text-left transition-all duration-300 hover:bg-white/12 active:scale-[0.97]"
              >
                <div
